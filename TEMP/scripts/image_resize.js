...
function resizePageImages() {
	...
	var elementNode = document.getElementById(contentId);
	if(document.all) {
		//IE expands the box, as such have to find the actual applied style
		elementWidth = elementNode.currentStyle['width'];
		elementWidth = elementWidth.replace('px','');
	} else {
		var elementWidth = elementNode.offsetWidth;
	}
	var imagesArray = elementNode.getElementsByTagName('img');
	for(i=0,n=imagesArray.length; i<n; i++) {
		var imageWidth = imagesArray[i].offsetWidth;
		if(imageWidth > elementWidth) {
			var imagesArrayStyle = imagesArray[i].style;
			//set styling behavior
			var imageHeight = imagesArray[i].offsetHeight;
			imagesArrayStyle['height'] = (imageHeight * ((elementWidth -30) / imageWidth)) + 'px';
			imagesArrayStyle['width'] = (elementWidth -30) + 'px';
			...
			if(imagesArray[i].id) {
				...
				imageId = generateId();
				imagesArray[i].id = imageId;
			}
			imagesArray[i].onclick = fullImageLayerOnClick;
			...
			imagesArray[i].parentNode.insertBefore(expandLink, imagesArray[i]);
